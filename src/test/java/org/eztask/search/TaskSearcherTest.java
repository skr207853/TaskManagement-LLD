...
class TaskSearcherTest {
    ...
    @Test
    @DisplayName("Should allow strategy to be immutable")
    void testStrategyIsImmutable() {
        // Arrange
        TaskSearchStrategy strategy = new TaskStatusSearchStrategy();
        searcher = new TaskSearcher(strategy);

        // Act & Assert
        // Strategy field is final, so it cannot be changed after construction
